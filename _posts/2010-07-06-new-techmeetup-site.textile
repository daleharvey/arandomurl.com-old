...
After a long time in waiting, the "new TechMeetup site":http://techmeetup.co.uk is now live. I will write a much longer and more interesting post once the dust has settled and we are in a routine, but for now will just quickly mention how it started and where it is going.

<a href="/random/techmeetup.png"><img src="/random/techmeetup.png" style="width:40em" /></a>

h2. Why we started it

The old techmeetup was a wordpress based system, wordpress was designed mainly to be a easy to setup blogging platform for those that couldnt (or didnt want to) write their own, it does its job incredibly well.

However for a community of awesome web talent it was a set of handcuffs, realistically only one person could do any work on the site, and when they did it had to be within wordpress's durastiction, hoping there was a plugin to do X and the Y plugin wouldnt break.
...
We decided to go with Django as it is popular within the community ("Peter Braden's talk on Django":http://vimeo.com/3770381), Oli wrote the first version with the blog, and not knowing Python or Django, I learnt them by building up the rest of the site (so excuse the nasty code, its probably mine). 

We started with everything on "github":http://github.com/daleharvey/techmeetup so you can fork the site right now and try out whatever you want.
...
If you have ideas for the site, new features or improvements, then please chime up at the next techmeetup or come along to the Royal Society where we will be working on it.
